...
def travel_salesman_problem(places, distances):
    movements = len(places) - 1  # Количество перемещений между точками.
    min_path = None  # Переменная для хранения самого короткого маршрута.
    min_path_length = maxsize

    for current_path in permutations(places):  # Цикл, перебирающий все возможные маршруты:
        current_path_length = 0

        for movement_index in range(movements):  # Цикл, вычисляющий расстояние для отдельного маршрута:
        ...
        if current_path_length < min_path_length:  # Если полученное расстояние меньше самого короткого пути...

            min_path_length = current_path_length  # ...назначаем полученное расстояние самым коротким.
            min_path = current_path  # Запоминаем самый короткий маршрут.

    return min_path_length, min_path # расстояние и самый короткий маршрут.
